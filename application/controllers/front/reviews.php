...
class reviews extends CI_Controller 
{
	...
	{
		if (file_exists(APPPATH . 'views/front/reviews.php'))
		{
			$this->load->model('posts_model');
			$this->load->model('category_model');
			
			//load the page if the load more doesn't exist
			...
			$this->template->load('front/reviews', 'template');
	...
	public function setHeader()
	{
		...
		$this->template->set('title','The Alternative Chronicle Reviews');
		...
		$this->template->set('meta_key', 'film, art, music, comics, television, books, comics');
	}
	
	public function setFeaturedMain()
	{
		$main_feature = $this->posts_model->getFeaturedMain('main-reviews');
	...
	public function setRecent()
	{
		...
		if ($recent !== '')
		{
			$this->template->set('recent_content', $this->setRecentReviews($recent));
		}
		else
		{
			echo FALSE;
	...
	public function loadMore($lastmsg)
	{
		...
		$more_content = $this->setRecentReviews($post_array);
